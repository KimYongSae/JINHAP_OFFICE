...
<%
	...
	String sFileName = "알람목록_" +sdf.format(now)+".xlsx";
	...
	{	
		...
		whereSql.append(" AND LEFT(timeact,10) BETWEEN '"+sdate+"' AND '"+edate+"' ");
		 
		s_sql.append("SELECT AreaMask, descString,  ");
		s_sql.append("LEFT(IFNULL(timeact,''),19) as timeact, ");
		s_sql.append("LEFT(IFNULL(timertn,''),19) as timertn ");
		s_sql.append("FROM v_ae ");
		s_sql.append("WHERE 1=1 ");
		s_sql.append(whereSql.toString());
		s_sql.append("ORDER BY timeact DESC, Areamask ");
		...
		objCell.setCellValue("발생시간"); objCell.setCellStyle(styleHd);
		
		objCell = objRow.createCell((short)4);
		objCell.setCellValue("해제시간"); objCell.setCellStyle(styleHd);		
		...
		objSheet.setColumnWidth((short)3,(short)12000);
		...
		while (rs.next()){
			...
			objCell = objRow.createCell((short)0);
			objCell.setCellValue((rsIdx)); objCell.setCellStyle(styleHd);
...
//			System.out.println(rsIdx+" : "+rs.getString("descString"));
			...
			objCell = objRow.createCell((short)3);
			objCell.setCellValue(rs.getString("timeact")); objCell.setCellStyle(styleHd);			

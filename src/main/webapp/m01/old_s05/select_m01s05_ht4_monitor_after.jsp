...
<%
	...
	sql.append("	AND hogi = '4' ");
	sql.append("	ORDER BY regtime DESC ");	
	sql.append(") AS b ");
	sql.append("WHERE 1=1 ");
	sql.append("AND num = 1 ");
	...
	try {
		...
		while(rs.next()){
			//이전정보
			bObj.put("n_lot", rs.getString("lot"));
			bObj.put("n_pnum", rs.getString("pnum"));
			bObj.put("n_gangjong", rs.getString("gang"));
			bObj.put("n_pname", rs.getString("pname"));
			bObj.put("n_t_gb", rs.getString("t_gb"));
			
			bObj.put("n_weight", rs.getString("weight"));
			bObj.put("n_stime", rs.getString("regtime"));	

			bObj.put("n_q1", rs.getString("q_temp"));
			bObj.put("n_q2", rs.getString("q_temp"));
			bObj.put("n_q3", rs.getString("q_temp"));
			bObj.put("n_q4", rs.getString("q_temp"));
			bObj.put("n_q5", rs.getString("q_temp"));
			
			bObj.put("n_t1", rs.getString("t_temp"));
			bObj.put("n_t2", rs.getString("t_temp"));
			bObj.put("n_t3", rs.getString("t_temp"));
			bObj.put("n_t4", rs.getString("t_temp"));
			
			bObj.put("n_cp", rs.getFloat("cp")*1000);
			...
			while(rs2.next()){
				bObj.put("n_tong_cnt", rs2.getInt("t_cnt"));
			}
		}
		...
		while(rs1.next()){
			...
			bObj.put("n_oil", rs1.getString("oil_sp"));

			
			bObj.put("n_a", rs1.getString("a_sp"));
			
			bObj.put("n_q_inv", "5000");
			bObj.put("n_t_inv", "4500");

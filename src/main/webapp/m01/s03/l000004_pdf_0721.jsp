...
//					reportMap.put("kg_hr",df.format(Math.round(rs_pop.getDouble("in_weight"))/rs.getInt("hr")*60));
				...
				erpSql.append("	AND A.END_TIME BETWEEN SYSDATE -10 AND SYSDATE+1 ");
				...
				erpSql.append("WHERE qt_lot_no = '"+rs.getString("heatno")+"' ");
				...
				while(rs_erp.next()){
					reportMap.put("qty",String.format("%,d",rs_erp.getInt("in_qty")));
					reportMap.put("weight",df.format( Integer.parseInt((Math.round(rs_erp.getDouble("in_weight"))+"")) ));
					
					double a = rs.getDouble("hr") / 60 ;
					double r_a = (Math.round(a*10) / 10.0);
					
					reportMap.put("kg_hr",df.format(Math.round((rs_erp.getDouble("in_weight")) ) / r_a ));
				}
...
//			 System.out.println("qun : "+rs3.getString("qun")+", tem : "+rs3.getString("tem")+", sign : "+rs3.getString("sign"));
			 gvo.setTdatetime(rs3.getString("datadate")+"0:00"); 
			 //:00

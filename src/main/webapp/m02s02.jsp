  ...
  <div style="width: 70%; " class="col-lg-12">
        <div class="box">
            <header>
                ...
                <h5 style="font-family: headline; font-size:14pt;">조건관리 - 열전대 교체이력</h5>
            </header>
...
//			alert("엑셀만해");
	...
	confirmDialog = $("#confirm-form").dialog({
		...
		buttons:{
			"삭 제":function(){
				$.post("m02/s02/delete_m02s02.jsp",{
					"cnt":fileDel_cnt
				},250);
	...
	function viewFile(c_val){
		$.ajax({
			type : "POST",
			url : "m02/s02/select_m02s02_file.jsp",
			cache : false,
			dataType : "json",
			data : {'time':new Date().getTime(),
					"cnt":c_val},
...
$("#thermo_contents").delegate('tr','click', function() {
	...
	$("#saveBtn").on("click",function(){
		...
		if(path != '' && path != null){
			...
			$.ajax({
				url:"m02/s02/insert_m02s02_file.jsp",
				type:"post",

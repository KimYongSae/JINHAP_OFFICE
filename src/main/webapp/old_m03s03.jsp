...
function init(){
 	...
 	function getYearCount(){
		...
		$.post("m03/old_s03/count_m03s03.jsp",{
			"tdate":$("#s_year").val()
		},100);
 	...
 	function getYearCount_bungi(){
		//년도를 기준으로 데이터베이스 검색해서 없으면 년도에 대해서 3~8호기 insert
		$.post("m03/old_s03/count_m03s03_bungi.jsp",{
			"tdate":$("#s_year").val()
		},100);
...
//			alert("엑셀만해");
	...
	if(path != ''){
		...
		$.ajax({
			url:"m03/old_s03/insert_m03s03_file.jsp",
			type:"post",
...
//			alert("엑셀만해");
	...
	if(path != ''){
		...
		$.ajax({
			url:"m03/old_s03/insert_m03s03_file_bungi.jsp",
			type:"post",
			enctype:"multipart/form-data",
			data:fileData,
			processData:false,
...
function getFileList(){
	$.ajax({
		type : "POST",
		url : "m03/old_s03/select_m03s03.jsp",
		cache : false,
		dataType : "json",
		data : {'time':new Date().getTime(),
			"year1":$("#s_year").val(),
...
function getFileList_bungi(){
	$.ajax({
		type : "POST",
		url : "m03/old_s03/select_m03s03_bungi.jsp",
		cache : false,
		dataType : "json",
		data : {'time':new Date().getTime(),
			"year1":$("#s_year").val(),
...
function fileOpen(c_val, gb){
	$.ajax({
		type : "POST",
		url : "m03/old_s03/select_m03s03_fileCheck.jsp",
		cache : false,
		dataType : "json",
		data : {'time':new Date().getTime(),
				"cnt":c_val,
...
function fileOpen_bungi(c_val, gb){
	$.ajax({
		type : "POST",
		url : "m03/old_s03/select_m03s03_fileCheck_bungi.jsp",
		cache : false,
		dataType : "json",
		data : {'time':new Date().getTime(),
				"cnt":c_val,
...
fileDialog = $("#file-form").dialog({
	...
	tdateDialog = $("#tdate-form").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_tdate.jsp",{
					"cnt":$("#t_cnt").val(),
					"gb":$("#t_gb").val(),
					"tdate":$("#t_date").val()
				},150);
	...
	chkDialog = $("#chk-form").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_chk.jsp",{
					"cnt":$("#c_cnt").val(),
					"gb":$("#c_gb").val(),
	...
	confirmDialog = $("#confirm-form").dialog({
		...
		buttons:{
			"삭 제":function(){
				$.post("m03/old_s03/delete_m03s03_file.jsp",{
					"cnt":fileDel_cnt,
					"gb":fileDel_gb
				},250);
	...
	memoDialog = $("#memo-form").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_memo.jsp",{
					"cnt":$("#m_cnt").val(),
					"gb":$("#m_gb").val(),
					"memo":$("#m_memo").val()
				},150);
...
//2020-12-15 분기
	...
	tdateDialog_bungi = $("#tdate-form_bungi").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_tdate_bungi.jsp",{
					"cnt":$("#t_cnt_bungi").val(),
					"gb":$("#t_gb_bungi").val(),
	...
	chkDialog_bungi = $("#chk-form_bungi").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_chk_bungi.jsp",{
					"cnt":$("#c_cnt_bungi").val(),
					"gb":$("#c_gb_bungi").val(),
	...
	memoDialog_bungi = $("#memo-form_bungi").dialog({
		...
		buttons:{
			"등 록":function(){
				...
				$.post("m03/old_s03/insert_m03s03_memo_bungi.jsp",{
					"cnt":$("#m_cnt_bungi").val(),
					"gb":$("#m_gb_bungi").val(),
	...
	confirmDialog_bungi = $("#confirm-form_bungi").dialog({
		...
		buttons:{
			"삭 제":function(){
				$.post("m03/old_s03/delete_m03s03_file_bungi.jsp",{

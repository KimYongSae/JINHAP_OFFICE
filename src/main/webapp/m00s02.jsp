  ...
  <div style="width: 50%; " class="col-lg-12">
        <div class="box">
            <header>
                ...
                <h5 style="font-family: headline; font-size:14pt;">모니터링 - 알람랭킹(월)</h5>
            </header>
...
<!-- 						<div class="form-group">
							...
							<div class="form-group">
								...
								font-size:13pt; font-weight:700; font-family:headline;
								padding-top:1px; padding-bottom:1px;" 
								id="m_searchbtn">
								<span class="glyphicon glyphicon-search"></span> 검 색</button>
							</div>
						</div> -->
               ...
               <table id="malarm_list" cellspacing="0" class="table table-bordered table-hover table-responsive scrolltbody">
					<thead>
						...
						<tr>
							...
							<th class="text-center cell" 
								style="background-color:#36FFFF; width: 400px; height: 40px; 
								font-size: 16pt; font-family:headline; font-weight:700;">
								알람내용
							</th>
							
							<th class="text-center cell" 
								style="background-color:#36FFFF; width: 70px; height: 40px; 
								font-size: 16pt; font-family:headline; font-weight:700;">
								발생수
							</th>							
						</tr>
 ...
 <!-- 연간 -->
   <div style="width: 50%; " class="col-lg-12">
        <div class="box">
            <header>
                <div style="padding:11px 14px;" class="icons"><i style="color:white;" class="fa fa-list-alt"></i></div>
                <h5 style="font-family: headline; font-size:14pt;">모니터링 - 알람랭킹(년)</h5>
            </header>
            <header style="background:white; height:43px;">
                <form class="form-inline" role="form" name="y_searchform" method="post" autocomplete="off">
						<!-- 설비 선택 -->
						<div class="form-group">
							<label class="control-label" 
							style="font-size: 14pt; font-family:headline; font-weight:700;
							 color:#8C8C8C; width: 50px; 
							text-align: right;">설비 : </label>
						</div>
						
						<div class="form-group">
							<select class="form-control input-sm datepicker" 
							id="y_hogi" name="y_hogi" 
							style="margin-top:5px; height: 30px; width: 120px; 
								font-size: 14pt; font-family:headline;font-weight:700;
								padding-top:1px; padding-bottom:1px;">
								<option value="0">전체</option>
            ...
            <div id="collapse4" class="body">
               <table id="yalarm_list" cellspacing="0" class="table table-bordered table-hover table-responsive scrolltbody">
					<thead>
...
function auto_date_format( e, oThis ){
	...
	function getMonthAlarmList(){
		$.ajax({
			type : "POST",
			url : "m00/s02/select_m00s02_month.jsp",
			cache : false,
			dataType : "json",
			data : {'time':new Date().getTime(),
					'hogi':$("#m_hogi").val(),
			...
			success : function(rsJson) {
				if (rsJson && rsJson.status == "ok") {
					...
					for(var i=0; i<rsAr.length; i++){
						listHtml += "<tr>";
						listHtml += '<td class="nr1" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 60px; word-break:break-all; font-size:15pt; font-family:headline;">'+(i+1)+'</td>';
						listHtml += '<td class="nr2" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 120px; word-break:break-all; font-size:15pt; font-family:headline;">'+rsAr[i].hogi+'</td>';
						listHtml += '<td class="nr3" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 400px; word-break:break-all; font-size:15pt; font-family:headline;">'+rsAr[i].descstring+'</td>';
						listHtml += '<td class="nr4" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 70px; word-break:break-all; font-size:15pt; font-family:headline;">'+rsAr[i].cnt+'</td>';
						listHtml += "</tr>";						
					}
					
					$("#malarm_list tbody").html(listHtml);
...
//				timer = setTimeout(function(){ o.run(); }, o.pollInterval);
	...
	function getYearAlarmList(){
		$.ajax({
			type : "POST",
			url : "m00/s02/select_m00s02_year.jsp",
			cache : false,
			dataType : "json",
			data : {'time':new Date().getTime(),
					'hogi':$("#y_hogi").val(),
			...
			success : function(rsJson) {
				if (rsJson && rsJson.status == "ok") {
					...
					for(var i=0; i<rsAr.length; i++){
						listHtml += "<tr>";
						listHtml += '<td class="nr1" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 60px; word-break:break-all; font-size:16pt; font-family:headline;">'+(i+1)+'</td>';
						listHtml += '<td class="nr2" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 120px; word-break:break-all; font-size:16pt; font-family:headline;">'+rsAr[i].hogi+'</td>';
						listHtml += '<td class="nr3" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 400px; word-break:break-all; font-size:16pt; font-family:headline;">'+rsAr[i].descstring+'</td>';
						listHtml += '<td class="nr4" style="text-align: center; vertical-align: middle; padding: 1px; height: 40px; width: 70px; word-break:break-all; font-size:16pt; font-family:headline;">'+rsAr[i].cnt+'</td>';
						listHtml += "</tr>";						
					}
					
					$("#yalarm_list tbody").html(listHtml);
...
/*이벤트*/	
	...
	$("#y_hogi").change(function(){
		getYearAlarmList();
	});
	
	$("#y_date").change(function(){
		getYearAlarmList();
	});	
	
	
	
	$("#m_excelBtn").on("click",function(){
		
		var form = document.forms["m_searchform"];
		
		form.action = "m00/s02/select_m00s02_excel_month.jsp";
		
		form.submit();
	});	

	$("#y_excelBtn").on("click",function(){
		
		var form = document.forms["y_searchform"];
		
		form.action = "m00/s02/select_m00s02_excel_year.jsp";
